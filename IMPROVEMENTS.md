...
- Статус (2026-03-07): частично закрыто.
...
- Статус (2026-03-07): закрыто кодом.
- Проблема: ошибки конфигурации обнаруживаются уже в runtime контейнера.
- Действие:
  - Добавить preflight-проверку в `manager.sh` (проверка обязательных переменных в `ENV_FILE`). ✅ Выполнено.
  - Явно логировать отсутствующие ключи перед `docker run`. ✅ Выполнено.
- Результат: более предсказуемый деплой и меньше аварийных запусков.

## Приоритет P1 (высокий)

### 3. Разграничение прав на команды управления
- Статус (2026-03-07): скорректировано.
...
- Статус (2026-03-07): закрыто кодом и документационно.
- Проблема: есть базовые логи, но нет структурной диагностики по шагам.
- Действие:
  - Добавить в `manager.sh` явные этапы и код завершения каждого шага. ✅ Выполнено.
  - Добавить краткую сводку успешного деплоя (image tag, env, время запуска). ✅ Выполнено.
...
- Статус (2026-03-07): закрыто документационно.
- Проблема: изменения деплоя и структуры иногда фиксируются только в коммитах.
- Действие:
  - Ввести короткий шаблон release notes на каждую заметную правку инфраструктуры. ✅ Выполнено (`RELEASE_NOTES.md`).
  - Добавить ссылку на него из `README.md`/`.github/ai_context.md`. ✅ Выполнено.
