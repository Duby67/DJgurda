...
- Действие:
  ...
  - Добавить единый `COOKIES_DIR` и auto-discovery `www.youtube.com_cookies.txt` / `instagram_cookies.txt` / `vk.com_cookies.txt` при пустых `*_COOKIES_PATH`. ✅ Выполнено (`src/config.py`, `env.example`, `README.md`, `.github/ai_context.md`).
  - Перевести локальный источник оригинальных cookies в `local/cookies`, добавить автокопирование в `src/data/cookies` для smoke-тестов handlers и использовать для `yt-dlp` только временные рабочие копии cookie-файлов (без модификации оригиналов). ✅ Выполнено (`local/sync_cookies.cmd`, `test/handlers/_local_cookie_setup.py`, `test/handlers/*/test_*_handlers_local.py`, `src/utils/cookies.py`, `README.md`, `.github/ai_context.md`, `env.example`).
