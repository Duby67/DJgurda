...
- Активные обработчики в runtime: TikTok, YouTube, Instagram, COUB (`ServiceManager` регистрирует `TikTokHandler`, `YouTubeHandler`, `InstagramHandler`, `CoubHandler`).
...
- `YOUTUBE_COOKIES_PATH` (опционально; используется при наличии валидного файла)

Условно-обязательные для Instagram cookies:

- `INSTAGRAM_COOKIES_ENABLED` (по умолчанию `true`)
- `INSTAGRAM_COOKIES_PATH` (опционально; используется при наличии валидного файла)
...
- Исключение для практической проверки handlers: используй локальные smoke-скрипты в `test/handlers/TikTok`, `test/handlers/YouTube`, `test/handlers/Instagram`, `test/handlers/Coub`.
...
- В deploy workflows `YOUTUBE_COOKIES_PATH` может задаваться опционально: `manager.sh` больше не требует этот ключ в preflight по умолчанию.
...
- `tests/` / `test/` - legacy зона на текущем этапе; не используется как источник актуального контекста, кроме согласованных smoke-скриптов handlers в `test/handlers/TikTok`, `test/handlers/YouTube`, `test/handlers/Instagram`, `test/handlers/Coub`.
...
7. Не считай `docs/` и `tests/` актуальным источником правды, кроме согласованных smoke-скриптов проверки handlers (`test/handlers/TikTok`, `test/handlers/YouTube`, `test/handlers/Instagram`, `test/handlers/Coub`).
