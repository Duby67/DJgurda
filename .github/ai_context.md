...
13. В конце обработки, после завершения всех правок, предлагай commit message на английском в формате готовой команды для cmd/PowerShell, например: git add . && git commit -a -m "docs: update release workflow and backlog sync".
14. Все выполненные задачи обязательно сверяй с `IMPROVEMENTS.md` и обновляй там статус; при релизной чистке выполненные задачи удаляй из активного backlog.
15. Для релизов с tag используй `python scripts/release_sync.py --tag <tag>` (и `--write` при необходимости обновить шаблоны файлов).
16. При запросе на выпуск новой версии действуй по процедуре:
