...
- `YOUTUBE_COOKIES_ENABLED` (по умолчанию `false`)
...
- При `YOUTUBE_COOKIES_ENABLED=false` файл cookies не обязателен и не используется YouTube handler.
- Если cookies-файл выглядит как заглушка (пустой/без cookie-строк), handler игнорирует его даже при включенном режиме.
...
- В deploy workflows путь `YOUTUBE_COOKIES_PATH` по-прежнему задается явно в `.env` для совместимости с `manager.sh`, даже если YouTube cookies отключены.
