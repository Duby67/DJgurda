...
- `PROJECT_CONTEXT.md` содержит полезный контекст, но частично устарел относительно кода.
...
Дополнительно:
- `PROJECT_CONTEXT.md` использовать только как исторический/вспомогательный контекст с обязательной сверкой по коду.
...
- `src/middlewares/db/` - DB-ядро, модели, миграции, операции.
...
- Жизненный цикл:
  - startup: DB init/migration, очистка temp, уведомления о запуске.
...
## Known Issues / Gaps
- `PROJECT_CONTEXT.md` частично устарел (есть несовпадения со структурой и фактическим runtime).
...
- `PROJECT_CONTEXT.md` - полезен для исторического контекста, но не источник истины без сверки.

## Instructions for AI Agents
1. Сначала читай `README.md`, `.github/ai_context.md`, `PROJECT_CONTEXT.md`.
2. Затем сверяй контекст по реальному коду.
3. Больше доверяй коду и config-файлам, чем старым markdown-документам.
4. Не считай `docs/` и `tests/` актуальным источником правды.
5. Не выдумывай отсутствующие фичи.
6. Если что-то непонятно - фиксируй это в разделе gaps, а не домысливай.
7. Вноси изменения минимально и локально, не ломая существующую структуру.
...
1. Синхронизировать `PROJECT_CONTEXT.md` с фактическим состоянием или пометить его как архивный.
2. Добавить `env.example`, согласованный с `src/config.py`.
3. Определить политику прав доступа к командам переключения (`/toggle_*`, `/start`, `/stop`).
4. Явно решить судьбу отключенных handlers (подключение или архивирование).
