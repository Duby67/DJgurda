...
- `deploy/` - deploy-артефакты репозитория (`deploy/Dockerfile`, `deploy/manager.sh`).
...
- `deploy/manager.sh` includes DB backup rotation in `$HOME/bot_{env}/data/db/backups` (retention: `DB_BACKUP_KEEP_COUNT`, default `14`).
- В deploy workflows cookies не копируются автоматически: `deploy/manager.sh` создает `$HOME/bot_{env}/data/cookies` и монтирует его в контейнер как `/app/src/data/cookies` (read-only).
...
17. Все выполненные задачи обязательно сверяй с `docs/improvements.md` и обновляй там статус; при релизной чистке выполненные задачи удаляй из активного backlog.
