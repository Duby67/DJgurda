...
- Активные стабильные обработчики в runtime: TikTok, YouTube, Instagram, COUB (`ServiceManager` регистрирует `TikTokHandler`, `YouTubeHandler`, `InstagramHandler`, `CoubHandler`).
...
- Обработчик Yandex Music присутствует в коде как legacy-зона и сейчас отключен в runtime.
...
- Yandex Music: SDK подключен в legacy handler-коде, но handler не входит в стабильный runtime.
...
- `YandexMusic` остается legacy-зоной и не входит в стабильный runtime.
...
2. Явно закрепить дальнейшую судьбу `YandexMusic` как legacy-зоны и вынести `VK` в отдельный R&D-трек без опоры на `yt-dlp`.
