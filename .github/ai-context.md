...
- `manager.sh` includes DB backup rotation in `$HOME/bot_{env}/data/db/backups` (retention: `DB_BACKUP_KEEP_COUNT`, default `14`).
...
5. Если работа ведется по `.github/ai-agent-technical-task.md`, все выполненные пункты нужно:
   ...
   - отразить в `IMPROVEMENTS.md`.
6. Перед внесением любых правок в код обязательно сверяйся с `IMPROVEMENTS.md`; если планируемые или уже выполненные изменения там не отражены, добавь их в backlog и только после этого продолжай работу.
7. Затем сверяй контекст по реальному коду.
8. Больше доверяй коду и config-файлам, чем старым markdown-документам.
9. Не считай `docs/` и `tests/` актуальным источником правды, кроме согласованных smoke-скриптов проверки handlers (`test/handlers/TikTok`, `test/handlers/YouTube`, `test/handlers/Instagram`, `test/handlers/Coub`, `test/handlers/VK`).
...
14. Язык комментариев и всех человеко-читаемых проектных файлов - русский.
15. Логи, технические runtime-сообщения и похожие служебные сегменты - английский.
16. В конце обработки, после завершения всех правок, предлагай commit message на английском в формате готовой для копирования, пример "docs: update release workflow and backlog sync".
17. Все выполненные задачи обязательно сверяй с `IMPROVEMENTS.md` и обновляй там статус; при релизной чистке выполненные задачи удаляй из активного backlog.
18. Для релизов с tag используй `python scripts/release_sync.py --tag <tag>` (и `--write` при необходимости обновить шаблоны файлов).
19. При запросе на выпуск новой версии действуй по процедуре:
20. При изменении `.md` файлов соблюдай единый стиль markdownlint: ориентируйся на `.markdownlint.json`, исключения из `.markdownlintignore` и перед финализацией проверяй `npx.cmd markdownlint-cli2 "**/*.md" ".github/*.md"`.
21. В пользовательских caption удаляй хэштеги из заголовков; если после очистки заголовок пустой, используй нейтральный fallback-текст вместо исходного набора хэштегов.
22. Если важный скрипт/тулза не запускается из-за ограничений среды (sandbox, права, сеть), запрашивай повышение прав у пользователя вместо молчаливого пропуска шага.
23. Для всех локальных Python-команд и проверок используй проектный `venv` (`.\venv\Scripts\python.exe` или активированный `venv`), не полагайся на системный Python.
24. Любые тесты (`pytest`, локальные smoke-скрипты handlers, интеграционные прогоны) запускай только с повышенными правами после явного запроса пользователю.
25. Rule: remote server access is forbidden for AI agents (SSH/RDP/WinRM/remote shell). Any server-side action is performed only by the user.
26. Encoding rule: UTF-8 with BOM is strongly discouraged; use plain UTF-8 without BOM.
